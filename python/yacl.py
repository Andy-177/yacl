...
class _Parser:
            ...
            if value_part:
                ...
                if next_indent > indent:
                    ...
                    if next_stripped.startswith('-'):
                        ...
                        value, consumed = self._read_multiline_string(indent + 2)
                        obj[key] = value
    ...
    def _read_multiline_string(self, indent_level: int) -> tuple:
        line = self.lines[self.index]
        stripped = line.lstrip()
        if stripped.startswith("'''"):
            quote = "'''"
        ...
        while self.index < len(self.lines):
            line = self.lines[self.index]
            if self._get_indent(line) < indent_level:
                break
    ...
    def _parse_inline_value(self, text: str, indent_level: int) -> Any:
        ...
        if low == 'none':
            return None
        if low in self.TRUE_VALUES:
            return True
        if low in self.FALSE_VALUES:
            return False
        try:
            if '.' in text:
                return float(text)
